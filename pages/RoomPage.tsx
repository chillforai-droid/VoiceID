...
export default function RoomPage() {
  ...
  const shareRoom = async () => {
    if (!room) return;
    const url = `${window.location.origin}/dashboard/rooms?code=${room.room_code}`;
    const text = `Join my room "${room.name}" on VoiceID! Code: ${room.room_code}`;
    if (navigator.share) {
      try { await navigator.share({ title: room.name, text, url }); } catch { /* user cancelled */ }
    } else {
  ...
  const shareMessage = async (msg: RoomMessage) => {
    try {
      if (msg.content_type === 'image' && (msg._previewUrl || msg.b2_object_key)) {
        const url = msg._previewUrl || await getRoomImageUrl(msg.id);
        const blob = await (await fetch(url)).blob();
        const file = new File([blob], 'image.jpg', { type: blob.type || 'image/jpeg' });
        if (navigator.canShare?.({ files: [file] })) {
          await navigator.share({ files: [file], text: msg.content || undefined });
          return;
        }
      }
      if (navigator.share) {
        await navigator.share({ text: msg.content || 'Shared from a VoiceID room' });
      } else {
  ...
  const goToProfile = (userId: string) => {
    if (userId) navigate(`/dashboard/profile/${userId}`);
  };
    ...
    <div className="flex flex-col h-full max-w-2xl mx-auto bg-white">
      ...
      <div className="relative bg-gradient-to-br from-indigo-600 via-purple-600 to-purple-700 px-4 pt-[calc(0.75rem+env(safe-area-inset-top,0px))] pb-4 shrink-0 overflow-hidden">
        <div className="flex items-center gap-3">
          ...
          <button onClick={shareRoom} className="p-2 text-white/90 hover:text-white shrink-0" aria-label="Share room">
            <Share2 size={20} />
          </button>
          <button
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-1.5 bg-gray-50">
        {loading ? (
          ...
          textMessages.map((msg: RoomMessage) => {
            ...
            return (
              <div key={msg.id} className="flex items-start gap-2 px-1.5 py-1 rounded-lg hover:bg-white/60 group">
                <button onClick={() => goToProfile(msg.sender_id)} className="shrink-0" aria-label="View profile">
                  {msg.sender?.avatar_url ? (
                    <img src={msg.sender.avatar_url} className="w-7 h-7 rounded-full object-cover mt-0.5" alt="" />
                  ) : (
                    <div className="w-7 h-7 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center text-[11px] font-bold mt-0.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-0.5 flex items-center gap-1.5">
                    <button onClick={() => goToProfile(msg.sender_id)} className={`text-sm font-semibold ${mine ? 'text-blue-600' : 'text-purple-700'}`}>
                    ...
                    <button
                      ...
                      className="ml-auto p-1 text-gray-300 hover:text-blue-600"
        ...
        <Sheet onClose={() => setShowMembers(false)} title="Members">
          <div className="space-y-2">
            {activeMembers.map(m => (
              <button
                key={m.id}
                onClick={() => { setShowMembers(false); goToProfile(m.user_id); }}
