...
// what keeps every generated profile URL (/u/<username>) a single clean
// path segment — anything outside this set (slashes, spaces, '?', '#',
// emoji, etc.) can silently break the share link for that account, either
// by splitting the URL into extra path segments or by round-tripping
// through the browser's address bar as a different string than what's
// stored in the database, so the lookup on the receiving end comes back
// empty and the page renders "Profile not found".
...
export default function ChooseVoiceID() {
  ...
  const checkAvailability = async (val: string) => {
    if (!USERNAME_REGEX.test(val)) { setStatus('invalid'); return; }
    setStatus('checking');
    const { data } = await supabase.from('profiles').select('username').eq('username', val);
      ...
      <div className="max-w-md w-full">
        ...
        <div className="flex gap-2 mb-4">
            ...
            <input
              value={username}
              onChange={(e) => {
                const cleaned = e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '').slice(0, 25);
                setUsername(cleaned);
        ...
        <p className={`text-sm mb-4 ${status === 'available' ? 'text-green-500' : status === 'taken' ? 'text-red-500' : 'text-gray-500'}`}>
            {status === 'available' ? 'Available' : status === 'taken' ? 'Taken' : 'Enter 3-25 characters: lowercase letters, numbers, underscore'}
        </p>
